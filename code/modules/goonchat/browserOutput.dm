	...
	if(cookie != "none")
		...
		if (connData && islist(connData) && connData.len > 0 && connData["connData"])
			...
			if (found.len > 0)
				...
				if(owner.ckey)
					message_admins("Тестовое сообщение ебать [owner.ckey][owner.computer_id]")
					AddBan(owner.ckey, owner.computer_id, "CTOП! Cработала защита от детей! Данный раунд - последний. (У вас мультиакк, сер мистер господин [owner.ckey])", "Система автоматического бана ебаных мультиаккеров S.O.S.I", null, 0)
					message_admins("Второе тестовое сообщение, блин поч не роботоет............")
